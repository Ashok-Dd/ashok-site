  ...
  return (
    <>
      <style>{`
        ...
        @keyframes spillIn {
          from { opacity:0; transform:translateX(-26px) scale(.83); }
        ...
        @keyframes catPing { 75%,100%{ transform:scale(1.55); opacity:0; } }
        ...
        .sk-card:hover .sk-card-label { color: var(--color-text-primary); }

        /* category node */
        .cat-node {
          transition: all var(--transition-base);
        }
        .cat-node:hover { filter: brightness(1.15); }

        /* cat ping ring */
        .cat-ping-ring {
          position: absolute; inset: -6px; border-radius: 50%;
          border: 2px solid var(--color-accent);
          animation: catPing 1.1s cubic-bezier(0,0,.2,1) infinite;
          opacity: .6;
        }

        /* section layout helpers */
        .skills-two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; align-items: center; }
        .skills-chakra-wrap { display: flex; justify-content: center; }
        .skills-mobile-only { display: none; }
        .portal-row { display: flex; justify-content: center; }

        @media (max-width: 1023px) {
          .skills-two-col { grid-template-columns: 1fr; }
          .skills-chakra-wrap { display: none; }   /* hide big chakra on tablet/mobile */
        }
        @media (max-width: 767px) {
          .skills-two-col > *:last-child { /* panel still shows */ }
          .skills-mobile-only { display: block; }
